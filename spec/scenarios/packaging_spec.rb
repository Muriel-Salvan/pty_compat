...
RSpec.describe 'Gem packaging' do
  it 'successfully builds the gem and creates the correct file in specific test location' do
    Dir.mktmpdir do |temp_dir|
      gem_file = File.join(temp_dir, "pty_compat-#{PtyCompat::VERSION}.gem")
      # Run gem build command with explicit output to our test directory
      stdout = `gem build pty_compat.gemspec --output #{gem_file}`
      ...
      expect(gem_spec.name).to eq('pty_compat')
      expect(gem_spec.version.to_s).to eq(PtyCompat::VERSION)
    end
  end
end
